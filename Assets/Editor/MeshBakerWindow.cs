...
namespace InsaneOne.DevTools
{
	public sealed class MeshBakerWindow : EditorWindow
	{
		...
		void ProcessTextures()
		{
			...
			isSpecularWorkflow = uniqueMaterials[0].HasProperty(specGlossMapId);
			...
			for (var q = 0; q < texturesCount; q++)
			{
				...
				// === Albedo ===
				if (uniqueMaterial.HasProperty(mainTexId))
					albedos[q] = uniqueMaterial.GetTexture(mainTexId) as Texture2D;

				if (albedos[q] == null)
				{
					Debug.LogWarning($"[MeshBaker] Skipping material '{uniqueMaterial.name}' because it has no Albedo texture or _MainTex property.");
					albedos[q] = Utils.CreateFilledTexture(32, 32, Color.magenta); // tiny magenta placeholder
				}
				...
				if (bakeSpecular)
				{
					int specId = isSpecularWorkflow ? specGlossMapId : metallicGlossMapId;

					if (uniqueMaterial.HasProperty(specId))
						speculars[q] = uniqueMaterial.GetTexture(specId) as Texture2D;

					if (!speculars[q])
						speculars[q] = Utils.CreateFilledTexture(width, height, new Color(0.5f, 0.5f, 0.5f, 1f));
				}
				...
				if (bakeNormals)
				{
					...
					if (uniqueMaterial.HasProperty(bumpMapId))
						loadedNormal = uniqueMaterial.GetTexture(bumpMapId) as Texture2D;
				...
				if (bakeAo)
				{
					if (uniqueMaterial.HasProperty(aoMapId))
		...
		void ResizeTextures(Texture2D[] textures, TextureType type)
		{
			...
			foreach (var texture in textures)
				if (texture && texture.width < minResolution)
